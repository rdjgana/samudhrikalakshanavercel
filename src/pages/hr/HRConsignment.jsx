...
const BOOKING_STATUS_NA = 'N/A'

const bookingStatusBadgeClass = (status) => {
  if (!status || status === BOOKING_STATUS_NA) return 'bg-gray-100 text-gray-600'
  const s = status.toLowerCase()
  if (s.includes('delivered')) return 'bg-green-100 text-green-800'
  if (s.includes('transit') || s.includes('dispatched')) return 'bg-blue-100 text-blue-800'
  if (s.includes('booked') || s.includes('confirmed') || s.includes('received')) return 'bg-amber-100 text-amber-900'
  if (s.includes('pending') || s.includes('await')) return 'bg-yellow-100 text-yellow-800'
  return 'bg-gray-100 text-gray-700'
}

const getBookingChainForNewEntry = (destination) => {
  switch (destination) {
    case 'To SS':
      return {
        bookingStatusSS: 'Booked',
        bookingStatusDistributor: 'Pending',
        bookingStatusShop: BOOKING_STATUS_NA,
      }
    case 'To Dist':
      return {
        bookingStatusSS: 'Handed over',
        bookingStatusDistributor: 'Booked',
        bookingStatusShop: BOOKING_STATUS_NA,
      }
    case 'To Shop':
      return {
        bookingStatusSS: 'Dispatched',
        bookingStatusDistributor: 'In transit to hub',
        bookingStatusShop: 'Booked',
      }
    case 'To Promoter':
    default:
      return {
        bookingStatusSS: BOOKING_STATUS_NA,
        bookingStatusDistributor: BOOKING_STATUS_NA,
        bookingStatusShop: BOOKING_STATUS_NA,
      }
  }
}
...
const MOCK_CONSIGNMENTS = [
  {
    ...
    photoUrl: '',
    bookingStatusSS: 'Dispatched',
    bookingStatusDistributor: 'Pending booking',
    bookingStatusShop: BOOKING_STATUS_NA,
  },
  {
    id: 2,
    ...
    status: 'In Transit',
    photoUrl: '',
    bookingStatusSS: 'Delivered to distributor',
    bookingStatusDistributor: 'In transit',
    bookingStatusShop: 'Pending',
  },
  {
    id: 3,
    orderDate: '2026-01-22',
    orderNo: 'ORD-003',
    partyName: 'Beauty Zone T Nagar',
    billValue: 95000,
    dispatchDate: '2026-01-23',
    trackingId: 'TRACK-003',
    destination: 'To Shop',
    status: 'In Transit',
    photoUrl: '',
    bookingStatusSS: 'Dispatched from SS',
    bookingStatusDistributor: 'Out for delivery',
    bookingStatusShop: 'Booked',
  },
]
...
const HRConsignment = () => {
  ...
  const handleBookingSubmit = () => {
    ...
    const chain = getBookingChainForNewEntry(bookingData.destination)
  ...
  const handleDownloadConsignments = (format = 'csv') => {
    ...
    const data = filteredConsignments.map(item => ({
      ...
      'Status': item.status,
      'SS booking status': item.bookingStatusSS ?? BOOKING_STATUS_NA,
      'Distributor booking status': item.bookingStatusDistributor ?? BOOKING_STATUS_NA,
      'Shop booking status': item.bookingStatusShop ?? BOOKING_STATUS_NA,
    }))
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Track consignments with booking status at super stockist, distributor, and shop levels
          </CardDescription>
        </CardHeader>
        <CardContent>
          {/* Filters Section */}
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>SS booking</TableHead>
                <TableHead>Distributor booking</TableHead>
                <TableHead>Shop booking</TableHead>
                ...
                <TableRow key={consignment.id}>
                  ...
                  <TableCell>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      consignment.status === 'Dispatched' || consignment.status === 'Delivered'
                  ...
                  <TableCell>
                    <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${bookingStatusBadgeClass(consignment.bookingStatusSS)}`}>
                      {consignment.bookingStatusSS ?? BOOKING_STATUS_NA}
                    </span>
                  </TableCell>
                  <TableCell>
                    <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${bookingStatusBadgeClass(consignment.bookingStatusDistributor)}`}>
                      {consignment.bookingStatusDistributor ?? BOOKING_STATUS_NA}
                    </span>
                  </TableCell>
                  <TableCell>
                    <span className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${bookingStatusBadgeClass(consignment.bookingStatusShop)}`}>
                      {consignment.bookingStatusShop ?? BOOKING_STATUS_NA}
                    </span>
                  </TableCell>
                  <TableCell>
                    <Button
      ...
      <Dialog open={showBookingDialog} onOpenChange={setShowBookingDialog}>
        <DialogContent className="max-w-md self-center sm:max-w-lg">
          <DialogHeader>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                ...
                <div className="md:col-span-2">
                  <p className="font-semibold text-gray-800 mb-2">Booking status (SS / Distributor / Shop)</p>
                  <div className="flex flex-wrap gap-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${bookingStatusBadgeClass(viewingConsignment.bookingStatusSS)}`}>
                      SS: {viewingConsignment.bookingStatusSS ?? BOOKING_STATUS_NA}
                    </span>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${bookingStatusBadgeClass(viewingConsignment.bookingStatusDistributor)}`}>
                      Distributor: {viewingConsignment.bookingStatusDistributor ?? BOOKING_STATUS_NA}
                    </span>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${bookingStatusBadgeClass(viewingConsignment.bookingStatusShop)}`}>
                      Shop: {viewingConsignment.bookingStatusShop ?? BOOKING_STATUS_NA}
                    </span>
                  </div>
                </div>
