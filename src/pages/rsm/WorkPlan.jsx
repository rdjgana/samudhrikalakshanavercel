...
const RSMWorkPlan = () => {
  ...
  const handleAddPlan = () => {
    if (!formData.date || !formData.fromPlace || !formData.fromPurpose || !formData.toPlace || !formData.toPurpose) {
      alert('Please fill all required fields: Date, From Place, From Purpose, To Place, and To Purpose')
      return
    }
  ...
  useEffect(() => {
    ...
    const userPlan = submittedPlans.find(
      plan => plan.employeeName === user?.name && plan.month === currentMonth && plan.status === 'Pending' && plan.employeeRole === 'RSM'
    )
    if (userPlan) {
      setIsSubmitted(true)
