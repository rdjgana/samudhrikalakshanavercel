...
const PromoterSalesReport = () => {
  ...
  const [productStocks] = useState(() =>
    MOCK_PRODUCTS.reduce((stocks, product) => {
      stocks[product.id] = Math.floor(Math.random() * 150) + 50;
      return stocks;
    }, {}),
  );

  const getAvailableStock = (productId) => {
    const initialStock = productStocks[productId] ?? 0;
    const cartQuantity = cart[productId]?.quantity ?? 0;
    return Math.max(initialStock - cartQuantity, 0);
  };
  ...
  const updateQuantity = (productId, newQuantity) => {
    ...
    const originalStock = productStocks[productId] ?? 0;
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-xs text-gray-600">
                              Stock: {getAvailableStock(item.product.id)} units
                            </p>
                          </div>
                          <Button
                            size="sm"
