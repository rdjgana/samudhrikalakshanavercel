...
const Activities = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    reason: '',
    photos: [],
    completionStatus: '', // For Primary Sales and Secondary Sales
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.voiceRecording) {
      setVoiceNoteError('Enter voice note')
      return
    }
    setVoiceNoteError('')
    ...
    if (isSupervisor) {
      // For Primary Sales and Secondary Sales, completion status is required
      if ((formData.purpose === 'Primary Sales' || formData.purpose === 'Secondary Sales') && !formData.completionStatus) {
  ...
  const renderActivityForm = () => (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {/* Completion Status for Primary Sales and Secondary Sales (Supervisor only) */}
      {isSupervisor && (formData.purpose === 'Primary Sales' || formData.purpose === 'Secondary Sales') && (
        <div className="space-y-2">
      ...
      <div className="space-y-3">
        ...
        {voiceNoteError && (
          <p className="text-sm text-destructive">{voiceNoteError}</p>
        )}
      </div>
                      ...
                      {isSupervisor && (
                        <TableCell>
                          {activity.issueCategory ? (
                            <span className="text-sm">{activity.issueCategory}</span>
                          ) : (
                            <span className="text-gray-400">N/A</span>
                          )}
                        </TableCell>
